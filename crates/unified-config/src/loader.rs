...
pub fn load_unified_config(
    ...
    let config_root = Path::new(governance_root).parent().unwrap_or_else(|| Path::new("."));
    ...
    let extensions_config =
        load_extensions_config(app_cfg.extensions_config_path.as_deref(), config_root)?;
...
fn load_extensions_config(
    ...
    let path = if path.is_absolute() { path } else { config_root.join(path) };
    if !path.exists() {
        return Ok(None);
    }
...
"#,
  ...
  "mcp_servers": {}
    ...
    #[test]
    fn test_load_unified_config_resolves_relative_extensions_path_from_config_root() {
        let dir = tempdir().unwrap();
        let config_root = dir.path().join("repo-root");
        let governance_root = config_root.join("governance");
        let extensions_path = config_root.join("extensions_config.json");
        std::fs::create_dir_all(&governance_root).unwrap();
...
"#,
        ...
        std::fs::write(
            ...
            r#"{
  "skills": {
    "review": { "enabled": true, "version": "1.0.0" }
  },
  "mcp_servers": {}
}"#,
        )
        ...
        let result = load_unified_config(
            &app_config_ref(Some("./extensions_config.json".to_string())),
            governance_root.to_str().unwrap(),
        )
        .unwrap();

        assert_eq!(result.policies.enabled_skills, vec!["review"]);
    }
