...
6. **Dispatch** — `classify_llm_routing` / `EngineCommand::try_from_llm_output` (see also `classify_turn_outcome`) → `dispatch::route_llm_output` + `build_dispatch_plan_with_options` → clarify / subagent / tools / text+memory branches, executed by `dispatch::execute_engine_command`, each with their own `LoopStage` and `commit_step` coordinated from `engine_v2.rs`.  
