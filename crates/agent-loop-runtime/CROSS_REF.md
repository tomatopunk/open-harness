...
| 概念 | harness 落点 |
|------|----------------|
| Pregel 超步 / 任务调度 | `run_agent_loop` 外层 turn + `execute_engine_command` 分支（可演进为显式任务队列） |
| State + reducer | `TurnEffect` + `StatePatch` 批量应用 |
| Checkpoint 元数据 | `CheckpointRecord::engine`（`EngineCheckpointExtensions`） |
| Command 路由 | `EngineCommand::from_llm_output` |
...
| 子任务 per-response 限额 | `min(max_subagent_tasks, 4)` 截断 |
